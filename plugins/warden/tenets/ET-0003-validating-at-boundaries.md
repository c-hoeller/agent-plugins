...
since: 0.1.0
triggers:
  - adding a null/range/format check inside an internal helper that already receives a typed argument
  - deciding where to validate input that crosses a trust boundary (HTTP handler, CLI args, queue message, deserializer)
  - reviewing a diff that scatters defensive guards through the call graph below the entry point
  - writing parsing/coercion logic that turns external data into a domain type
tags: [validation, boundaries, defensive-programming, types]
related: []
---
