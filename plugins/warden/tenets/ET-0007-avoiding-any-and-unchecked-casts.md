...
since: 0.1.0
triggers:
  - typing a value as `any` to make a TypeScript error go away
  - writing `as Foo` to coerce a value of unknown shape into a domain type
  - turning untyped JSON / network / form data into a typed value without runtime validation
  - silencing a type error with `// @ts-ignore` or `// @ts-expect-error` to keep the build green
paths:
  - "**/*.ts"
  - "**/*.tsx"
tags: [typescript, types, validation, casts]
related: [ET-0003]
---
