---
id: ET-0004
title: Tests assert observable behavior, not implementation
type: best-practice
severity: high
tier: 1
applies-to: any
since: 0.1.0
triggers:
  - writing a test that verifies a mock was called instead of the effect the call produced
  - reaching for a spy/stub/inspector to peek at internal state from a test
  - reviewing a test that breaks every time the production code is refactored
  - deciding what to assert about a function whose contract is "given X, the world looks like Y"
tags: [testing, mocks, refactoring, behavior]
related: [ET-0001]
---
