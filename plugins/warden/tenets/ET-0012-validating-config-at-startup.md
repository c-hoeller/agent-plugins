---
id: ET-0012
title: Load and validate configuration at startup, not at first use
type: best-practice
tier: 1
applies-to: any
since: 0.2.0
triggers:
  ...
  - reviewing a diff that defers config parsing into a request handler / job runner / command body
  - deciding when to validate that required config (URLs, secrets, feature flags, durations) is present and well-formed
tags: [configuration, startup, validation, fail-fast]
related: [ET-0003, ET-0011]
---
