...
since: 0.1.0
triggers:
  - handling an error you do not know how to recover from in the current scope
  - writing a `catch` / `except` / `.catch()` block that does nothing or only logs
  - reviewing a diff that adds a try/catch around new code without rethrow or remediation
  - deciding what to do with a Promise/Task whose failure path has no obvious owner
