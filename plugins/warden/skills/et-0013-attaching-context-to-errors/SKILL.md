---
name: et-0013-attaching-context-to-errors
description: Attach diagnostic context to errors; preserve the cause chain.
when_to_use: Use when raising or throwing an exception with only a string message; logging an error that names the failure but not the inputs / IDs needed to reproduce it; reviewing a diff where the only diagnostic information is the exception class name; deciding which fields a domain error type should expose to its handler.
user-invocable: false
---
<!-- generated from tenets/ET-0013-attaching-context-to-errors.md by `uv run poe build` — do not edit by hand. -->

# ET-0013 — Attach diagnostic context to errors; preserve the cause chain
