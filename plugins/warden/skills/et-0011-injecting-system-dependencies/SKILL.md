---
name: et-0011-injecting-system-dependencies
description: Inject time, randomness, and IO; never read them ambiently.
when_to_use: Use when calling `DateTime.UtcNow` / `Date.now()` / `datetime.now()` inside business logic; calling `Random` / `Math.random()` / `random.random()` / `Guid.NewGuid()` to make a decision domain code depends on; needing to wait, sleep, or freeze the clock in a test because production code reads the wall clock; reviewing a diff where logic branches on "now" or on a freshly-generated ID without taking either as a parameter.
user-invocable: false
---
<!-- generated from tenets/ET-0011-injecting-system-dependencies.md by `uv run poe build` — do not edit by hand. -->

# ET-0011 — Inject time, randomness, and IO; never read them ambiently
