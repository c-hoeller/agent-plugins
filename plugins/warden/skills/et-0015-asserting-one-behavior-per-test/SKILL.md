---
name: et-0015-asserting-one-behavior-per-test
description: Assert one behavior per test.
when_to_use: Use when writing a test whose body covers two unrelated behaviors with the same setup; naming a test `test_login_and_signup_and_password_reset` or similar conjunction-named test; reviewing a test where the failure message could mean any of several different broken things; deciding whether multiple `expect` / `Assert` calls belong in one test or in several.
user-invocable: false
---
<!-- generated from tenets/ET-0015-asserting-one-behavior-per-test.md by `uv run poe build` — do not edit by hand. -->

# ET-0015 — Assert one behavior per test
